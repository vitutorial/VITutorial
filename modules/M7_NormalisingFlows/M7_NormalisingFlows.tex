...
\begin{frame}{Recap: Change Of Variable}
\begin{block}{What if a function is multivariate}
...
\pause
\begin{block}{Problem}
If we want to express $ p(y) $ in terms of $ p(x) $, we need to provide $ \djac{h^{-1}}{y} $ \textbf{in the forward pass}. But that's hard!
\end{block}
...
\begin{frame}{Inverse Autoregressive Flows: Density Estimation}
\begin{small}
\begin{block}{Setting}
...
\begin{frame}{Inverse Autoregressive Flow: Inference}
\begin{equation*}
\begin{aligned}
\ELBO &= -\KL{q(z|x)}{p(z|x)} \\
