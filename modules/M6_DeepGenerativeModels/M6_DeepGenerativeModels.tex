...
&= \frac{d}{d\lambda} \intl{\alert{q(\epsilon)} \loga{ p(x | \alert{\overbrace{h(\epsilon, \lambda)}^{=z}},\theta)
\alert{\overbrace{\frac{d}{d\epsilon}h(\epsilon, \lambda)}^{\text{constant if h linear}}}}}{\alert{\epsilon}} \\ \pause
...
\begin{equation*}
\begin{aligned}
\E[p(\epsilon)]{\frac{d}{d\lambda} \log p(x| h(\epsilon, \lambda), \theta)\times \frac{d}{d\lambda} h(\epsilon, \lambda)}\\
\overset{\text{MC}}{\approx} \frac{1}{S}\sum_{i=1}^{S} \frac{d}{d\lambda} \log p(x| h(\epsilon_{i}, \lambda), \theta) \times \frac{d}{d\lambda} h(\epsilon, \lambda)
\end{aligned}
\end{equation*}
\end{frame}
