...
\begin{equation*}
\begin{aligned}
h = \frac{1}{N} \sum_{i=1}^N E_{x_i} \qquad b_1^K = \sigmoid( W h + b)
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
	&\KL{q(z|x, \lambda)}{p(z|\phi)}  = \sum_{k=1}^K \KL{q(z_i|x, \lambda)}{p(z_i|\phi)} \\
...
\pdv{\theta} \log p(x|z^{(k)},\theta) \quad \textcolor{gray}{\text{where } z^{(k)} \sim q(z|x,\lambda)}
\end{aligned}
\end{equation*}
...
&\overset{\text{MC}}{\approx} \frac{1}{K} \sum_{k=1}^K \alert{\log p(x|z^{(k)}, \theta)} \pdv{\lambda}\log q(z^{(k)}|x, \lambda) \\
&\textcolor{gray}{\text{where } z^{(k)} \sim q(z|x, \lambda)}
\end{aligned}
\end{equation*}
\end{small}
...
\pause
\node[circle, draw, thick, above right= of loss] (z) {$ z $};
...
\pause
\node[rectangle, fill=blue!20, thick, above of= psi, rounded corners, draw, node distance=2cm] (scorefunction) {$ \log q_\lambda(z|\psi) \uncover<5->{\alert{\log p_\theta(x|z)}} $};
\node[rectangle, fill=blue!20, thick, above right= of input, rounded corners, draw] (loss) {$ x $};
...
\begin{equation*}
\hat f \overset{\text{MC}}{\approx} \frac{1}{S} \sum_{s=1}^S f(z_s)
\end{equation*}
...
In general
\begin{equation*}
\Var(\hat d) = \Var(\hat f) - 2\Cov(\hat f, \hat \psi) + \underbrace{\Var(\hat \psi)}_{0 \text{ by assumption}}
\end{equation*}
