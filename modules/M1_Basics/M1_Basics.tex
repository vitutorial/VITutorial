...
\usepackage{amsmath, amssymb, ../../vimacros, hyperref, enumerate, subcaption}
...
$$ p(x|\mu, \sigma) \propto \intl{\exp\left(w^{T}x\right)p(w|\mu, \sigma)}{z} $$
\begin{figure}
\begin{tikzpicture}
\node[obs] (x) {$ y $};
...
\end{tikzpicture}
\end{figure}
The functional form of the posterior is unknown.
\end{frame}
...
\alert{Approximate Posterior: } $ q(z_{1},\ldots, z_{K}) = \prod_{k=1}^{K} q(z_{k}) $

%At inference time the latent variables are marginally dependent. For our variational distribution
%we are going to assume that they are not (recall: mean field assumption).
\end{frame}

\section{Amortized VI}
...
We have $K$ latent variables and $ D $ data points \\
\begin{itemize}
	\item assume the posterior factorises as $KD$ independent terms
\end{itemize}


\begin{equation*}
q(z_1, \ldots, z_KD) = \underbrace{\prod_{j=1}^{KD} q(z_j|\alert{\lambda_j})}_{\text{mean field}}
\end{equation*} \pause
...
\foreach \x in {1,...,4} {
\pgfmathtruncatemacro{\y}{\x-1}
\ifthenelse{\x=1}{\node[obs] (x\x) {$ x_{\x} $}}{\node[obs, right= of x\y] (x\x) {$ x_{1\x} $}};
}
\foreach \z in {1,2,3} {
  \node[latent, above right = of x\z] (z\z) {$ z_{1\z} $};
  \edge[color=red]{x1,x2,x3,x4}{z\z};
  \node[above = of z\z] (lambda\z) {$ \lambda_{1\z} $};
...
\foreach \x in {1,...,4} {
\pgfmathtruncatemacro{\y}{\x-1}
\ifthenelse{\x=1}{\node[obs] (x\x) {$ x_{\x} $}}{\node[obs, right= of x\y] (x\x) {$ x_{2\x} $}};
}
\foreach \z in {1,2,3} {
  \node[latent, above right = of x\z] (z\z) {$ z_{2\z} $};
  \edge[color=red]{x1,x2,x3,x4}{z\z};
  \node[above = of z\z] (lambda\z) {$ \lambda_{2\z} $};
  \edge[color=red]{lambda\z}{z\z};
}
\end{tikzpicture}
\end{subfigure}
...
\begin{equation*}
q(z_1, \ldots, z_{KD}|x) = \prod_{j=1}^{KD} q(z_j|\text{NN}_{\alert{\lambda}}(x)=b_{j})
\end{equation*} \pause
~ still mean field
$$Z_j|x \sim \Bernoulli(b_j)$$\\  \pause
~ but with a shared set of parameters
\begin{itemize}
	\item where $b_1^K = \text{NN}_{\alert{\lambda}}(x)$ 
\end{itemize}

\end{frame}

\begin{frame}{Amortized mean field: example}
\begin{figure}[t!]
\begin{subfigure}[t]{.4\textwidth}
\begin{tikzpicture}
\foreach \x in {1,...,4} {
