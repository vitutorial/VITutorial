...
\end{equation*}
\pause
\item Maximize the expected log-density:
\begin{equation*}
...
\pause
\begin{enumerate}
\item Compute $ \E[p(z|x)]{\loga{p(x,z)}} $
\pause
\item $ \underset{p(x,z)}{\max}~\E[p(z|x)]{\loga{p(x,z)}} + \underbrace{\Ent{p(z|x)}}_{\text{constant}} $
\end{enumerate}
\end{frame}
...
If $ q(z) = p(z|x) $ then $ \KL{q(z)}{p(z|x)} = 0 $ and thus we are directly optimising the log-evidence.
\begin{itemize}
\item[\alert{E-step}] Compute $ \E[p(z|x)]{\loga{p(x,z)}} $.
