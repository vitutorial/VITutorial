...
\pause
\begin{block}{Idea}
Let's approximate it by an auxiliary distribution $ q(z|x) $ that is computable!
\end{block}
\pause
...
\begin{itemize}
\pause
\item $ \KL{q(z|x)}{p(z|x)} = \intl{ q(z|x) \loga{\frac{q(z|x)}{p(z|x)}}}{z} $ (continuous)
\pause
\item $ \KL{q(z|x)}{p(z|x)} = \sum_{z} q(z|x) \loga{\frac{q(z|x)}{p(z|x)}} $ (discrete)
\pause
\item $ \KL{q(z|x)}{p(z|x)} = \E[q(z|x)]{\loga{\frac{q(z|x)}{p(z|x)}}} $ (both)
\end{itemize}
\end{frame}

\begin{frame}{Recap KL divergence}
Originally known as relative entropy. Read $ \KL{q(z|x)}{p(z)} $ as the divergence of $ p $ from $ q $ or the entropy of $ p $ relative to $ q $.
\pause
\begin{small}
\begin{block}{Expand $ \KL{q}{p} $}
\begin{align*}
\E[q(z|x)]{\loga{\frac{q(z|x)}{p(z)}}} &= - \E[q(z|x)]{\log p(z)} + \E[q(z|x)]{\log q(z|x)} \\
&\onslide<3->{= \text{CE} - \Ent{q(z|x)}}
\end{align*}
\end{block}
\onslide<4>
\end{small}
...
\begin{itemize}
\item $ \KL{q(z|x)}{p(z|x)} \geq 0 $ with \\ equality iff $ q(z|x) = p(z|x) $.
\pause
\item $ -\KL{q(z|x)}{p(z|x)} = \E[q(z|x)]{\loga{\frac{p(z|x)}{q(z|x)}}} \leq 0 $.
\pause
\item $ \KL{q(z|x)}{p(z|x)} = \infty $ \\ if $ \exists z $ s.t. $ p(z|x) = 0 $ and $ q(z|x) > 0 $.
\pause
\item In general $ \KL{q(z|x)}{p(z|x)} \not = \KL{p(z|x)}{q(z|x)} $.
\end{itemize}
\end{block}
\end{frame}
...
\pause
&= \loga{\intl{\alert{q(z|x)}\frac{p(x,z)}{\alert{q(z|x)}}}{z}} \\
\pause
&\geq \intl{ \alert{q(z|x)} \loga{\frac{p(x,z)}{\alert{q(z|x)}}}} {z} \\
\pause
& = \intl{ \alert{q(z|x)} \loga{\frac{p(z|x)p(x)}{\alert{q(z|x)}}}} {z} \\
\pause
&= \intl{ \alert{q(z|x)} \loga{\frac{p(z|x)}{\alert{q(z|x)}}}} {z} + \log p(x)
\end{aligned}
\end{equation*}
\end{small}
\end{frame}
...
\begin{equation*}
\begin{aligned}
\log p(x) &\geq \intl{ q(z|x) \loga{\frac{p(z|x)}{q(z|x)}}}{z} + \log p(x) \\
\pause
...
Recall that we want to find $ q(z|x) $ such that $ \KL{q(z|x)}{p(z|x)} $ is small.
\pause
\begin{block}{Formal Objective}
\begin{equation*}
\underset{q(z|x)}{\min} \KL{q(z|x)}{p(z|x)} \pause = \underset{q(z|x)}{\max} -\KL{q(z|x)}{p(z|x)}
\end{equation*}
\end{block}
\end{frame}
...
\begin{equation*}
\begin{aligned}
&\underset{q(z|x)}{\max} -\KL{q(z|x)}{p(z|x)} \\ 
\pause
&= \underset{q(z|x)}{\max} \intl{ q(z|x) \loga{\frac{p(z|x)}{q(z|x)}}}{z} \\
\pause
&= \underset{q(z|x)}{\max} \intl{ q(z|x) \loga{\frac{p(z,x)}{p(x)q(z|x)}}} {z} \\
\pause
&= \underset{q(z|x)}{\max} \intl{ q(z|x) \loga{p(z,x)}} {z} - \intl{ q(z|x) \log q(z|x)} {z} 
- \overbrace{\log p(x)}^{constant} \\
\pause
&= \underset{q(z|x)}{\max}~\E[q(z|x)]{\log p(x,z) } + \Ent{q(z|x)}
\end{aligned}
\end{equation*}
\end{small}
\end{frame}
...
\begin{equation*}
\underset{q(z|x)}{\max}~\E[q(z|x)]{\loga{p(x,z)}} + \Ent{q(z|x)}
\end{equation*}
\pause
\item Optimise generative model.
\begin{equation*}
\underset{p(x,z)}{\max}~ \E[q(z|x)]{\loga{p(x,z)}} + \underbrace{\Ent{q(z|x)}}_{\text{constant}}
\end{equation*}
\end{enumerate}
\end{frame}
...
\begin{equation*}
\underset{q(z|x)}{\max}~\E[q(z|x)q(\theta)]{\log p(x,z,\theta)} + \Ent{q(z|x)}
\end{equation*}
\pause
\item Maximise over global variables $ \theta $.
\begin{equation*}
\underset{q(\theta)}{\max}~ \E[q(z|x)q(\theta)]{\log p(x,z,\theta)} + \Ent{q(\theta)}
\end{equation*}
\end{enumerate}
\end{frame}
...
\begin{itemize}
\item Recall: The approximation $ q(z|x) $ needs to be tractable.
\item Common solution: make \textbf{all} latent variables independent under $ q(z|x) $.
\pause
\item Formal assumption: $ q(z|x) = \prod_{i=1}^{N}q(z_{i}) $
\end{itemize}
\pause
...
\item Variational inference approximates the posterior $ p(z|x) $ with a simpler distribution $ q(z|x) $.
\item The variational objective is the \textbf{evidence lower bound (ELBO)}:
\begin{equation*}
\E[q(z|x)]{\loga{p(x,z)}} + \Ent{q(z|x)}
\end{equation*}
\end{itemize}
\end{frame}
