...
\begin{equation*}
\begin{aligned}
\log p(x) &\geq \intl{ q(z) \loga{\frac{p(z|x)}{q(z)}}}{z} + \loga{p(x)} \\
\pause
...
\begin{itemize}
\item[\alert{E-step}] Compute: $ \E[p(z|x)]{\loga{p(x,z)}} $. \\ Same as: $ \underset{p(z|x)}{\max}~ \E[p(z|x)]{\loga{p(x,z)}} $
\item[\alert{M-step}] $ \underset{p(x,z)}{\max}~\E[p(z|x)]{\loga{p(x,z)}} + \underbrace{\Ent{p(z|x)}}_{\text{constant}} $
\end{itemize}
\pause 
...
We split $ z $ into local variables $ z_{loc} $ (e.g. POS tags) and global variables $ z_{glob} $ (e.g. model parameters).
\begin{enumerate}
\item Maximise over local variables $ z_{loc} $.
\begin{equation*}
\underset{q(z_{loc})}{\max}~\E[q(z)]{\loga{p(x,z_{loc},z_{glob})}} + \Ent{q(z_{loc})}
\end{equation*}
\pause
\item Maximise over global variables $ z_{glob} $.
\begin{equation*}
\underset{q(z_{glob})}{\max}~ \E[q(z)]{\loga{p(x,z_{loc},z_{glob})}} + {\Ent{q(z_{glob})}}
\end{equation*}
\end{enumerate}
\end{frame}
