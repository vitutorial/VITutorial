...
\end{itemize}
\pause
This model is \textbf{intractable} because of the exponential dependence on the number of hidden Markov chains.
\end{frame}
...
Make $ q(z) $ as close as possible to $ p(z|x) $ to obtain a faithful approximation.
\end{block}
\pause
...
a distribution $ q $. 
\begin{itemize}
\item $ \KL{q(z)}{p(z|x)} = \int q(z) \loga{\frac{q(z)}{p(z|x)}} dz $ (continuous)
...
\pause
\item $ -\KL{q(z)}{p(z|x)} = \int q(z) \frac{p(z|x)}{q(z)} dz \leq 0 $.
\end{itemize}
\end{block}
\end{frame}
...
\begin{small}
\begin{align*}
\loga{p(x)} &= \int \loga{p(x,z)} dz \\
&= \int \loga{\alert{q(z)}\frac{p(x,z)}{\alert{q(z)}}} dz \\
&\geq \int \alert{q(z)} \loga{\frac{p(x,z)}{\alert{q(z)}}} dz \\
& = \int \alert{q(z)} \loga{\frac{p(z|x)p(x)}{\alert{q(z)}}} dz \\
&= \int \alert{q(z)} \loga{\frac{p(z|x)}{\alert{q(z)}}} dz + \loga{p(x)}
\end{align*}
\end{small}
\end{frame}
...
\begin{small}
\begin{align*}
\int q(z) \loga{\frac{p(z|x)}{q(z)}} dz + \loga{p(x)} \\
= -\KL{q(z)}{p(z|x)} + \loga{p(x)}
\end{align*}
\end{small}
confirming that we have derived a lower bound on the log-evidence. The gap in the bound is exactly
the KL divergence between the model posterior $ p(z|x) $ and its approximation $ q(z) $.
\end{frame}
...
&= \underset{q(z)}{\max} \int q(z) \loga{\frac{p(z|x)}{q(z)}} dz \\
&= \underset{q(z)}{\max} \int q(z) \loga{\frac{p(z,x)}{p(x)q(z)}} dz \\
&= \underset{q(z)}{\max} \int q(z) \loga{p(z,x)} dz - \int q(z) \loga{q(z)} dz 
- \overbrace{\log(p(x))}^{C} \\
...
\item The implementation of the M-step does not change.
\item The implementation of the E-step changes when $ q(z) \not = p(z|x) $ because the expectation term
changes.
\begin{equation*}
...
\end{tikzpicture}
\end{figure}
\center{Approximate posterior $ q(s,z) =q(s)q(z)$}
\end{frame}
...
\end{tikzpicture}
\end{figure}
\center{Approximate posterior $ q(z,\theta|w,\alpha, \beta) = q(z|w,\beta)q(\theta|w,\alpha) $}
\end{frame}
