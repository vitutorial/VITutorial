...
\pause
&= \loga{\E[q(z)]{\frac{p(x,z)}{\alert{q(z)}}}} \\ \pause
&\geq \E[q(z)]{  \log{\frac{p(x,z)}{\alert{q(z)}}}} \\
\pause
...
\begin{aligned}
\log p(x) &\geq 
\E[q(z|x)] {\log{\frac{p(z|x)p(x)}{q(z)}}} \\
\pause
&= \intl{ q(z) \log{\frac{p(z|x)}{q(z)}}}{z} + \log p(x) \\
\pause
...
&\underset{q(z)}{\argmax} -\KL{q(z)}{p(z|x)} \\ 
\pause
&= \underset{q(z)}{\argmax} \intl{ q(z) \log{\frac{p(z|x)}{q(z)}}}{z} \\
\pause
&= \underset{q(z)}{\argmax} \intl{ q(z) \log{\frac{p(z,x)}{p(x)q(z)}}} {z} \\
\pause
&= \underset{q(z)}{\argmax} \intl{ q(z) \log{p(z,x)}} {z} - \intl{ q(z) \log q(z)} {z} 
...
\end{equation*}
if $\mathcal Q$ includes the true posterior $p(z|x)$?
...
\begin{itemize}
\item[\alert{E-step}] Compute: $\log p(x)$ \\ 
Same as: $ \underset{p(z|x)}{\max}~ \E[p(z|x)]{\log p(x, z)} + \Ent{p(z|x)} $ \pause
...
\end{aligned}
\end{equation*} 
$ f(\cdot) $ is computed by a NN with softmax output.
\end{frame}
...
\begin{itemize}
	\item where $b_1^K = g(x; \alert{\lambda})$ 
\end{itemize}
