...
\begin{itemize}
	...
	\item the categorical distribution in turn depends on Gaussian latent factors $ z = (z_{1},\ldots,z_{K}) $ which are also i.i.d. \pause
\end{itemize}

\begin{equation*}
\begin{aligned}
z_{j} &\sim \mathcal N(0, 1) && (1 \leq j \leq K) \\ 
...
with independent sets of parameters $\lambda_j = \{u_j, s_j\}$
...
\end{equation*} \pause
~ still mean field
$$Z_j|x \sim \mathcal N(u_j, s_j^2)$$\\  \pause
~ but with a shared set of parameters
\begin{itemize}
	\item where $u_1^K = \mu_{\alert{\lambda}}(x)$ 
	\item and $s_1^K = \sigma_{\alert{\lambda}}(x)$
\end{itemize}
