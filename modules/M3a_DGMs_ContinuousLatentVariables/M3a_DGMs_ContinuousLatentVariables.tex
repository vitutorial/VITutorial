...
\usetikzlibrary{positioning, fit, bayesnet, patterns, shapes.misc, patterns}
...
%    \begin{equation*}
%	\begin{aligned}						
%		p(x_1^N|z, \theta) &= \prod_{i=1}^N p(x_i|z, \theta) \pause = \prod_{i=1}^N \text{Bernoulli}(x_i|\underbrace{f(z, \theta)}_{=\psi}) \\ \pause
%		&= \prod_{i=1}^N \psi^{x_i}(1-\psi)^{1-x_i}
%	\end{aligned}
%	\end{equation*}
%	\end{small}
%
...
%	\begin{equation*}
%	\begin{aligned}		
%		\E[q\left(\frac{z-u}{s}\right)]{ \textstyle\sum_{i=1}^N \log P(x|z,\theta) \\
