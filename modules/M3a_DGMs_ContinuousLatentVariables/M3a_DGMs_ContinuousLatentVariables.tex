...
\begin{equation*}
\begin{aligned}
&\frac{d}{d\lambda}\E[q(z|x,\lambda)]{\log p(x|z,\theta)} \\ \pause
&= \frac{d}{d\lambda} \intl{q(z|x,\lambda)\log p(x|z,\theta)}{z} \\ \pause
&= \intl{\alert{\frac{d}{d\lambda} q(z|x,\lambda)}\log p(x|z,\theta)}{z}
\end{aligned}
\end{equation*}
\pause
...
\begin{equation*}
\begin{aligned}
&\E[q(\epsilon)]{\frac{d}{d\lambda} \log p(x| \overbrace{h^{-1}(\epsilon, \lambda)}^{=z}, \theta)} \\ \pause
...
\pause
\begin{align*}
\frac{d h^{-1}(\epsilon, \lambda)}{d\mu(\phi,\lambda)} = \frac{d}{d\mu(\phi,\lambda)}\left[ \mu(\phi,\lambda) + \sigma(\phi,\lambda) \odot \epsilon \right] = 1 \\
\frac{d h^{-1}(\epsilon, \lambda)}{d\sigma(\phi,\lambda)} = \frac{d}{d\sigma(\phi,\lambda)}\left[ \mu(\phi,\lambda) + \sigma(\phi,\lambda) \odot \epsilon \right] = \epsilon
\end{align*}
\end{frame}
